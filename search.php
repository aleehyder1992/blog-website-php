...
$sql="SELECT * FROM blog LEFT JOIN catagories ON blog.catagory=catagories.cat_id LEFT JOIN user ON blog.author_id=user.user_id WHERE blog_title like '%$keyword%' or blog_body like '%$keyword%' ORDER BY blog.publish_date DESC limit $offset,$limit";
...
<div class="container mt-2">
	<div class="row">
    <h3>
      Search Result For: <span class="text-danger">
        <?php echo $keyword; ?>
      </span>
    </h3>
		<div class="col-lg-8">
      ...
      <?php for ($i=1; $i <=$pages ; $i++) { ?>
			...
			<li class="page-item">
		  <a href="search.php? keyword=<? $keyword ?> pages=<?= $i ?>" class="page-link <?php if($i==$pages) { echo $active="active";
      }else{
	    echo "";
      }  ?>">
		 <?= $i ?>
